...
import type { onChange } from 'node_modules/react-toastify/dist/core/store';
...
export const Form = () => {
  ...
  const inputs = [
    ...
    },
    {
      ...
      showToggle: false,
      onChange: (e:  ChangeEvent<HTMLInputElement>) => setValue('document', e.target.value),
    },
    {
      id: 'password',
      label: 'Senha',
  ...
  return (
    <motion.form       initial={{ opacity: 0, y: 10 }}
      ...
      {inputs.map(({ id, label, type, placeholder, showToggle, toggleState, onToggle, mask, onChange }) => (
        <div key={id} className="space-y-2">
          ...
          {errors[id as keyof SignUpFormData] && (
            <p className="text-sm text-red-500">{errors[id as keyof SignUpFormData]?.message}</p>
          )}
        </div>
      ))}
